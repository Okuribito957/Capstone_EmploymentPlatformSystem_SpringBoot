<template>
    <div>
        <div class="btn-panel">
            <el-button size="small" type="primary" icon="el-icon-plus" @click="showDetail">详情</el-button>
        ...
        <Detail v-if="detail.visible" :param="detail"></Detail>
    </div>
</template>
...
<script>
    ...
    export default {
        name: "Send",
        components: {RPagination, RTable, Detail},
        data() {
            return {
                total: 0,
                page: 1,
        ...
        methods: {
            ...
            showDetail() {
                ...
                if (selections.length === 1) {
                    sessionStorage.setItem("session_send", JSON.stringify(selections[0]));
                    this.detail.visible = true;
                    this.detail.form = selections[0];
                } else {
                    message.warning('请选择一条数据查看详情');
                }
            },
            del() {
